...
from sbstudio.plugin.light_fx_presets import (
...
class ColorFunctionProperties(PropertyGroup):
    ...
    # Built-in light preset reference (used when output type is "PRESET").
    # Stores only a stable string ID, so projects are portable across
    # machines and operating systems (no absolute file path).
    ...
    def update_from_dict(self, data: dict[str, Any]) -> None:
        ...
        if name := data.get("name"):
            self.name = name
        if preset_id := data.get("preset_id"):
            try:
                self.preset_id = preset_id
            except (TypeError, ValueError):
                # Preset no longer exists in this version of the addon;
                # silently fall back to the empty default so we don't break
                # loading of older or newer .blend files.
                pass
    ...
    def apply_on_colors(
        ...
        *,
        ...
        def get_output_based_on_output_type(
            ...
            elif output_type == "PRESET":
                # Built-in, fully-portable light effect preset.
                # Resolved by string ID, no .py file path involved.
                fn = get_preset_function(output_function.preset_id)
                if fn is not None:
                    outputs = [
                        fn(
                            frame=frame,
