...
_zh_translations = {
        ...
        ("*", "4-Layer"): "4层",
        ("*", "9-Layer"): "9层",
        # === Starlight Transition Editor (Storyboard panel) ===
        ...
        # === Starlight Return To Home (RTH Panel) ===
        ("*", "Return Group"): "返航分组",
        # === Takeoff extra properties ===
        ("*", "with Velocity"): "速度",
        ("*", "to Altitude"): "高度",
        ("*", "Layer height"): "层高",
        ("*", "Mixed Mode"): "混合模式",
...
# Register same translations under both Blender 4.x (zh_HANS) and older (zh_CN) codes
translations_dict = {
...
def get_box_offsets(orientation):
    """Return the 8 box offsets for the given orientation."""
...
def create_box_preset_grid(
    ...
    custom_offsets: 可选，自定义箱内坐标列表 [(x,y), ...]
    drones_per_box: 每箱无人机数（默认8）
    """
...
# === Box Preset Update Functions ===

_UPDATE_LOCK = False  # Prevent recursive updates


# === Mixed Position bilingual items ===

_mixed_position_items_cache = {}


def _mixed_position_items_cb(self, context):
    """Callable items for mixed_position EnumProperty - 中英日三语支持。
    
    必须缓存返回的列表（Blender 要求 items callable 返回稳定引用，否则可能崩溃）。
    """
    try:
...
def _distribute_boxes_to_grid(boxes):
    ...
    # 优化：尝试找出使 rows*cols 最接近 boxes 的组合
    best_cols = cols
    best_rows = rows
    ...
    for c in range(max(1, cols - 2), cols + 3):
        r = math.ceil(boxes / c)
        diff = r * c - boxes
        # 优先选择正好等于 boxes 的组合，否则选择差值小且接近正方形的
...
# === Box Preset Import/Export Functions ===
...
class CreateTakeoffGridOperator(Operator):
    ...
    box_drone_count = IntProperty(
        name="Drone Count",
        description="Total number of drones in the grid",
        default=48,
        min=0,
        soft_max=10000,
    ...
    # 行数和列数不带 update 回调 - 修改时不联动，仅在点击 apply 时联动
    ...
    # 行列应用按钮（BoolProperty toggle，update 时执行联动）
    ...
    # 每架无人机的 XY 坐标 - 使用 FloatVectorProperty(size=2) 支持直接编辑
    # 默认值对应 BOX_OFFSETS_VERTICAL (8架/箱)
    ...
    def _is_chinese(self):
        """判断 Blender 当前语言是否为中文"""
        return self._get_locale() == "zh"

    def draw(self, context):
        layout = self.layout
        layout.use_property_split = True
        layout.ui_units_x = 18  # Increase UI width to 150%
        ...
        # 选择分区图标（Blender 内置图标）
        ICON_SINGLE = 'MESH_GRID'   # 网格状小点 - 单机阵列
        ICON_BOX = 'PACKAGE'        # 方框形 - 箱式阵列
        ICON_MIXED = 'OVERLAY'      # 叠加 - 混合模式
        ...
        if self.use_advanced_settings:
            # ========== 箱式阵列 ==========
            box = layout.box()
            # 关闭 property_split，手动控制标签和输入框宽度
            box.use_property_split = False
            header = box.row()
            header.label(text=L["box_array"], icon=ICON_BOX)
            
            # 辅助函数：创建 label + prop 行，标签左对齐，输入框固定宽度
            ...
            relation_row.enabled = False  # 浅色显示（非交互提示）
    ...
    def _run(self, context):
        ...
        def _generate_traditional_points(center_x, center_y):
            """用顶部基础参数生成传统网格，以 (center_x, center_y) 为中心"""
            pts = create_points_of_takeoff_grid(
                center=(center_x, center_y, cz),
                rows=self.rows,
                columns=self.columns,
                num_drones_per_slot_row=1,
                num_drones_per_slot_col=1,
                spacing_row=self.spacing,
                spacing_col=self.spacing,
                intra_slot_spacing_row=0.5,
                intra_slot_spacing_col=0.5,
            )[: self.drones]
            return list(pts)
        
        def _generate_box_points(center_x, center_y):
            """用箱起预设生成网格，以 (center_x, center_y) 为中心"""
            custom_offsets = None
        ...
        def _bounding_size(pts):
            ...
            xs = [p[0] for p in pts]
            ys = [p[1] for p in pts]
            return (max(xs) - min(xs) + drone_diameter,
        ...
        # Determine group tag for each drone index (based on mode)
        box_n = int(scene.get("sb_mixed_box_count", 0))
        trad_n = int(scene.get("sb_mixed_trad_count", 0))
        def _group_for_index(i: int) -> str:
            # In mixed mode: first box_n are BOX, rest are TRAD
            # In box-only mode: all BOX
            # In traditional/simple mode: all TRAD
        ...
        # Adjust takeoff grid marker size if using box preset (advanced mode)
        if self.use_advanced_settings and takeoff_grid:
            # Set empty display size to 35% of default
...
class ExportBoxPresetOperator(Operator):
    ...
    def execute(self, context):
        # Get current box orientation
        wm = context.window_manager
        if hasattr(wm, 'skybrush_box_orientation'):
            orientation = wm.skybrush_box_orientation
        else:
...
class SetDroneCoordOperator(Operator):
    ...
    coord_type: StringProperty()  # 'X' or 'Y'
    value: FloatProperty()
    ...
    def draw(self, context):
        layout = self.layout
        layout.prop(self, "value", text=f"{self.coord_type} Coordinate")
    
    def execute(self, context):
        # Store the coordinate change in window manager
        wm = context.window_manager
...
class ApplyGridLayoutOperator(Operator):
    ...
    def execute(self, context):
        # Get the current operator properties from window manager
        wm = context.window_manager
...
class ApplyBoxParamsOperator(Operator):
    ...
    def execute(self, context):
        ...
        # We need to update the global constants and drones_per_box
        wm = context.window_manager
        
        # Store the edited parameters
        # These will be read by the CreateTakeoffGridOperator
